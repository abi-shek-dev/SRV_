...
import { normalizeClassValue } from '../utils/pollUtils.js';
...
router.get('/:grade/:section', protect, async (req, res) => {
  try {
    const grade = normalizeClassValue(req.params.grade);
    const section = normalizeClassValue(req.params.section);
    const periods = await Timetable.findByClass(grade, section);
...
router.get('/today/:grade/:section', protect, async (req, res) => {
  try {
    ...
    const grade = normalizeClassValue(req.params.grade);
    const section = normalizeClassValue(req.params.section);
    const periods = await Timetable.findByClassAndDay(grade, section, today);
...
router.post('/', protect, adminOnly, async (req, res) => {
  ...
  try {
    const nGrade = normalizeClassValue(grade);
    const nSection = normalizeClassValue(section);
    const result = await Timetable.bulkUpsert(nGrade, nSection, periods);
