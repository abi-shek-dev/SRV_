...
export function FacultyStudentLeavesSection() {
  ...
  const fetchLeaves = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_URL}/api/faculty/leave-requests`, { headers: { Authorization: `Bearer ${token}` } });
      setLeaves(res.data);
    } catch (error) {
      console.error(error);
    } finally {
  ...
  const displayedLeaves = leaveFilter ? leaves.filter(l => l.status === leaveFilter) : leaves;

  const handleAction = async (id, status) => {
    try {
      const token = localStorage.getItem('schoolToken');
      await axios.put(`${API_URL}/api/faculty/leave-requests/${id}`, { status, reviewNote }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  return (
    <div className="space-y-6">
      ...
      {displayedLeaves.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-14 text-center text-slate-500">No student leave requests found.</div>
        ...
        <div key={leave._id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {leave.reviewNote && <p className="mt-2 rounded-lg bg-slate-50 p-3 text-sm text-slate-600"><span className="font-bold">Faculty Note:</span> {leave.reviewNote}</p>}

          {leave.status === 'PENDING' && (
            <div className="mt-4 space-y-3">
              {reviewingId === leave._id ? (
                <>
                  <input type="text" placeholder="Optional note for parent..." value={reviewNote} onChange={e => setReviewNote(e.target.value)}
