...
export function FacultyMyLeavesSection() {
  ...
  const fetchLeaves = async () => {
    try {
      ...
      const res = await axios.get(`${API_URL}/api/faculty/my-leaves`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('schoolToken');
      await axios.post(`${API_URL}/api/faculty/my-leaves`, form, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="lg:col-span-1 space-y-6">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <h3 className="mb-4 text-lg font-display font-bold text-slate-900 flex items-center gap-2">
            <CalendarDays className="text-indigo-600" /> Request Leave
          </h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <select 
                ...
                className="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 outline-none focus:ring-2 focus:ring-indigo-500"
              >
            ...
            <button 
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-bold text-white transition hover:bg-indigo-700 active:scale-95 disabled:opacity-70"
            >
      ...
      <div className="lg:col-span-2 space-y-4">
        {loading ? (
           <div className="flex justify-center py-20"><div className="h-8 w-8 animate-spin rounded-full border-4 border-indigo-500 border-t-transparent"></div></div>
        ) : leaves.length === 0 ? (
            ...
            <div key={leave._id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              {leave.reviewNote && (
                <div className="mt-3 rounded-lg bg-slate-50 p-3 text-sm text-slate-700">
                  <span className="font-bold text-slate-900 text-xs uppercase tracking-wider block mb-1">Admin Note</span>
                  {leave.reviewNote}
                </div>
              )}
            </div>
